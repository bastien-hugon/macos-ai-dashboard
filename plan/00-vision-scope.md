...
| **P3 — Le dev « full local »** | Refuse tout outil qui exfiltre ses prompts, transcripts ou diffs ; audite le trafic réseau de ses apps | Garantie zéro télémétrie, zéro compte, réseau limité à 4 destinations désactivables, tout l'état sur la machine (§12 features) |
...
| 10 | Onboarding, trial 48 h, licence one-time, distribution DMG signé/notarisé + Sparkle | §11 | distribution/licensing |

Sur le point 10 : la section 14 des features le laissait « à décider » ; **la décision est actée** dans `plan/01-architecture.md` (A7, A11 : LicensingKit, Sparkle, trial 48 h anti-rollback, Worker d'activation). Ce document l'entérine — le clone reproduit aussi le modèle économique (essai 2 jours, licence à vie, « No subscription, ever »).
...
| Réseau | Optionnel — toutes les fonctions locales marchent hors ligne ; seuls usage Claude/Cursor, mises à jour et activation de licence en dépendent | Test hors ligne |
...
2. **Zéro télémétrie, zéro compte, zéro analytics** : le produit ne sait rien de ses utilisateurs ; l'activation de licence transmet uniquement clé + hash machine + version d'app.
...
| **Trial / high-water mark** | Essai 48 h à double stockage anti-rollback (l'horloge ne peut pas reculer le compteur). |
| **Reçu de licence** | Preuve d'activation signée Ed25519, vérifiée hors ligne à chaque lancement. |
| **What's New** | Fenêtre post-mise à jour (validable avec Entrée). |
| **Appcast** | Flux Sparkle des mises à jour (checks horaires). |
