...
| 13 | notch **et** menu bar désactivés, ou trial expiré (notch verrouillé) | Comportement `.terminalOnly` forcé : libération immédiate, l'agent garde la main. Jamais de prompt retenu sans surface pour y répondre. |
...
4. **Hotkeys** — consommation de ⌘A depuis un `NSPanel` non activant à confirmer par test UI (question ouverte n°5 de system-integration) ; conflits avec les launchers répandus (REQ-ACT-26 les rend non bloquants).
