...
- **Gatekeeper/quarantaine sur le binaire copié** hors bundle — premier passage en build de dev signée ad hoc (hypothèse system-integration n°4 ; verdict définitif en M8 sur la build de release signée).
...
| Onglet About (structure, logs, acknowledgements ; texte de mise à jour manuelle finalisé en M8) | `13 · S13/S14, REQ-SET-66, 69/70, 72+` |
...
- `REQ-SET` P0 verts (hors parties Cursor différées en M7 et About/mise à jour manuelle finalisés en M8) ; `REQ-NUI` P0/P1 restants verts.
...
## M8 — Onboarding + packaging one-shot (release 0.1.0)

**Objectif :** première release distribuable selon le modèle « one-shot » (décision produit du 3 juillet 2026) — onboarding welcome (≤ 3 interactions jusqu'à la première session), app signée et packagée en DMG (Developer ID si le certificat est disponible, sinon signature ad hoc documentée), mise à jour par **remplacement manuel** de `AgentDash.app`, zéro infrastructure distante propre.

### Tâches

| Tâche | Référence |
|---|---|
| `OnboardingCoordinator` + fenêtre Welcome (welcome → détection des agents → installation des hooks → permission notifications → terminé) | `14 · T1/T2/T3, REQ-LIC-01..06, 08..10` ; `12 · T6` |
| Mise à jour manuelle : resynchronisation par hash de `~/.agentdash/bin` au premier lancement d'une nouvelle version, survie des réglages et des `.bak`, gestion du remplacement à chaud (app en cours d'exécution) | `14 · REQ-LIC-51+ (nouvelles exigences)` ; `01 · A2` |
| About final : version, logs (export + dossier), acknowledgements, texte de mise à jour manuelle | `13 · S13/S14, REQ-SET-66, 69/70, 72+` |
| Pipeline build/signature : codesign Developer ID si le certificat est disponible, sinon signature ad hoc documentée ; vérification `codesign --verify --deep --strict` | `14 · REQ-LIC-39 (révisée one-shot)` |
| Notarisation + staple + DMG `create-dmg` + CI GitHub Actions `macos-26` — **P2** | `14 · REQ-LIC-40/41 (séquence réduite, révisées one-shot), REQ-LIC-43/44 (limitées à la CI et aux secrets de signature)` |
| Versionnage SemVer + `scripts/bump-version.sh` + `CHANGELOG.md` | `14 · REQ-LIC-46` |
...
| Audit final privacy : 24 h derrière proxy, **2 destinations exactement** (`api.anthropic.com`, `cursor.com`), toggles off = silence | `00 · REQ-VIS-12/13` ; `15 · REQ-TST-41 (9)` |

### Spikes

- **Gatekeeper sur le binaire copié en build de release** — verdict définitif de l'hypothèse system-integration n°4 (en build Developer ID notarisée si le certificat est disponible, sinon sur le parcours ad hoc documenté) ; plan B : hooks `type:"http"` pour Claude (`01 · §2`).
- **Remplacement de `AgentDash.app` pendant que l'app tourne** (avertissement du Finder, relance, resynchronisation du binaire hook) — banc des nouvelles exigences `14 · REQ-LIC-51+`.
- Prompt notifications/fenêtre d'onboarding au premier plan en `LSUIElement` (D-L n°9).

### Livrable démontrable

Sur un Mac Apple Silicon vierge (macOS 14) : monter le DMG → glisser-déposer dans `/Applications` → lancement (aucune invite Gatekeeper en build notarisée ; parcours ad hoc documenté sinon) ; onboarding `Continue` → `Install hooks` → permission notifications = **≤ 3 interactions**, hooks « Ready », première session Claude visible < 2 min ; remplacer `AgentDash.app` par la version N+1 dans `/Applications` → à la relance, réglages intacts, `.bak` préservés, `~/.agentdash/bin` resynchronisé (hash vérifié), Doctor vert ; Settings → About affiche la nouvelle version et le texte de mise à jour manuelle.

### Critères de sortie

- Les **10 gates de release** (`15 · REQ-TST-41`) tous verts, dont : QA P0 sur ≥ 2 OS (dont macOS 14), Doctor vert sur machine vierge, capture réseau conforme (**2 destinations**), **remplacement manuel N→N+1 validé** (réglages et `.bak` intacts, binaire hook resynchronisé).
- `00 · REQ-VIS-23` (time-to-value) et `REQ-VIS-27` (distribution) démontrés.
- Aucune hypothèse restante `[HYPOTHÈSE]` ne conditionne une feature annoncée (`15 · REQ-TST-41 (10)`).

### Risques

Certificat Developer ID absent au moment de la release → distribution en signature ad hoc avec parcours d'installation documenté (clic droit → Open, ou `xattr -d com.apple.quarantine`), notarisation/DMG/CI restant en P2 ; risque juridique/commercial du clone (`00 · R6`) — nom, marque et visuels propres à trancher avec `00 · T1` **avant** toute distribution publique.
...
| **M8** Onboarding + packaging one-shot | 0.1.0 | 14, 13 (About), 12 (étape onboarding), 15 (release), 00 (audits) | REQ-LIC-01..06, 08..10 (P0, onboarding) · REQ-LIC-39..41, 43/44, 46 (P1, pipeline révisé one-shot) · REQ-LIC-51+ (mise à jour manuelle) · REQ-VIS-05, 07, 12/13, 23, 25 (validation finale) · REQ-TST-34, 37, 39..41 | **Gatekeeper build de release (verdict)** ; remplacement à chaud N→N+1 ; prompt onboarding `LSUIElement` |

Rappels transverses valables à chaque jalon : budgets `01 · A9` (RAM < 150 Mo, CPU idle < 0,5 %, hook → UI < 150 ms) vérifiés au banc ; fail-open (`01 · A10`) non négociable ; privacy (`00 · REQ-VIS-12/13`) contrôlée dès qu'une destination réseau s'ajoute (M3, M7) puis auditée en clôture (M8 — 2 destinations exactement, opt-out) ; matrice de traçabilité (`00 · T2`) mise à jour à la clôture de chaque jalon.
